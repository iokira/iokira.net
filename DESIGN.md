...
| 2026-08-19 | 資格を趣味として再分類 | 「資格は趣味」というユーザーの位置づけにより、本・曲・コーヒーと合わせて4カテゴリの「趣味」ページ (別ページ、未実装) にまとめる方針に変更。第一段階としてトップページから「資格」節を削除し、トップページ構成は 見出し部 → Bio のみに。`qualifications` 配列と構造化データの `hasCredential` は表示先が変わるだけなので維持 |
